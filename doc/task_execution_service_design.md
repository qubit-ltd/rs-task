...
执行引擎必须把处理器的返回错误、panic 和基础设施启动失败区分开。启动失败时释放资源、记录可诊断原因，并按明确的有限重试策略重新排队或标记失败；不能让任务永久占有资源。业务返回错误默认是终态 `Failed`，本期不自动重试，避免无意重复副作用。处理器可以主动返回可重试的基础设施错误，是否重试及最大次数由服务配置。
